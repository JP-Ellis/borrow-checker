...
The importer is a **pure parsing concern** — it converts bytes to `RawTransaction` values. Accounts live **on the postings**: each `RawTransaction` carries one or more `RawPosting` legs, and every leg names its own account **path** (e.g. `Assets:NAB:Josh`). Multi-account formats (Ledger, Beancount) name each leg's account directly; single-account formats (CSV, OFX) emit exactly one leg whose account path comes from the importer's own config blob. A leg's `amount` is optional — `None` marks an elided residual that balances the transaction. Account **path → id** resolution happens later, in `bc-core` at persistence time.
