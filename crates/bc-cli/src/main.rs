...
async fn main() {
    ...
    let result: crate::error::CliResult<()> = match cli.command {
        ...
        Commands::Backup(args) => commands::backup::execute(args, &ctx).await,
        ...
        Commands::Sync(args) => commands::sync::execute(args, &ctx).await,
        ...
        Commands::Meta(args) => commands::meta::execute(args, &ctx).await,
