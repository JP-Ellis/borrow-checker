...
//!
//! This module provides [`ImporterFactory`], a lightweight `Copy + Clone` descriptor
...
/// A lightweight, copyable descriptor for a single importer format.
///
...
///
/// Because all three fields are `Copy` types, `ImporterFactory` is itself `Copy`.
/// External code must use [`ImporterFactory::new`] to construct instances (the
/// `#[non_exhaustive]` attribute prevents struct-literal construction outside this
/// crate).
///
/// # Example
///
